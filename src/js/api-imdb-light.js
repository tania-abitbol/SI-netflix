...
function apiCallLight(list, j= 0) { //list = sliderItem (look the call function)
  if (j === listlen) { // Stop function
    return
  }
  let listlen = list.length;
    fetch(`http://www.omdbapi.com/?apikey=${apiKey}=${encodeURI(films)}`)
      ...
      .then(response => {
        ...
        if (image !== "N/A" && typeof image !== "undefined") { //verification validity response image
